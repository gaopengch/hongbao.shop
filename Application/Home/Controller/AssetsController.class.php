...
class AssetsController extends BaseController{
     ...
     public function __construct()
    {
        ...
        $this->isLogin();
        $this->integral = D('integralUse')->valid($_SESSION['user_id'],0,ture);
    }
    //余额
    ...
    public function coupon_del(){
        $where['id'] = I('post.id/d');//用户领取的优惠券id
    ...
    public function collection_del(){
        $m  = M('Collection');
        $id = I('post.id/d');
        $u_id = $m->field('user_id')->where(['id'=>$id])->find();
        $res = $m->where('id='.$id)->delete();
        $uid = number_format($u_id['user_id']);
        if (!$res) {
            $data['msg'] = 0;

            $this->ajaxReturn($data);
          }else{
            $count = $m->where(['user_id'=>$uid])->count();
            $data['count'] = $count;
            $data['msg'] = 1;
            $this->ajaxReturn($data);
        }
    ...
    public function integral(){
        ...
        foreach ($Data['res'] as $key => $value) {
            if(empty($value['goods_id'])){
                $goods_null[$key]['title']        = $value['remarks'];
                $goods_null[$key]['images']       = '/static/new_xin.png';
                $goods_null[$key]['type']         = $value['type'];
                $goods_null[$key]['goods_id']     = $value['goods_id'];
                $goods_null[$key]['integral']     = $value['integral'];
                $goods_null[$key]['trading_time'] = $value['trading_time'];
                $goods_null[$key]['remarks']      = $value['remarks'];
        ...
        $data = GoodsImagesModel::getGoodsImageByData($Goods);
        //合并数组
        if(!empty($goods_null) && !empty($data)){
            $data  =  array_merge($goods_null,$data);
        }else if(!empty($goods_null) && empty($data)){
            $data = $goods_null;
        }
        $this->assign('integral',$this->integral);
        $this->assign('data',$data);
        $this->assign('type',1); 
