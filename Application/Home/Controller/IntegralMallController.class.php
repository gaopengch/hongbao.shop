...
class IntegralMallController extends BaseController
{
    ...
    public function index()
    {
        ...
        if ($user_id > 0) {
            ...
            $info['integral']    = D('integralUse')->valid($user_id);
    ...
    public function confirm()
    {
        ...
        // 获取购物车ID list
        $id_str   = addslashes(I('POST.cart_id', -1));
        ...
        // 应付金额
//        $this->total_price =  $goods_info['total_money'] + $goods_carry;   //注释掉，修改前端显示支付金额  ---meng
        $this->total_price =  $goods_info['total_money'] ;
        $_SESSION['user_goods_monery'] = $goods_info['total_money'];         //设置商品总价，计算运费需要用  ---meng
    ...
    public function pay()
    {
        ...
        $data  = [
            ...
            'user_id'        => $user_id, // 用户
            'create_time'    => time(),
            'order_status'   => $order_status,      // 未支付
            'comment_status' => 0,      // 未评论
            'pay_type'       => 5,      // 支付类型  5为积分支付
            'remarks'        => $message, // 订单备注
            'status'         => 0,      // 订单正常
            'translate'      => 0,      // 0,不需要发票   1,需要发票
            'separate_order' => 1,      // 1为积分订单 
        ];
        ...
        $ret = M('goods')->where(['id'=>$goods['goods_id']])->setDec('stock',$goods['goods_num']);
        if ($ret < 1) {
            $trans->rollback();
            $this->error('订单失败');
        }
        $ret = M('specGoodsPrice')->where(['goods_id'=>$goods['goods_id']])->setDec('store_count',$goods['goods_num']);
        if ($ret < 1) {
            $trans->rollback();
            $this->error('订单失败');
        }
        ...
        //总费用等于0,则不需要支付
        if($total != 0){
           //生成数组方便增加积分             这段意为 会员在商城消费的钱*后台设置比例，添加到积分使用表  貌似没用到  ---meng
