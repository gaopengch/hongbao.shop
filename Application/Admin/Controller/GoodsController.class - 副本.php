...
//	        'url' => U('AjaxGetAttribute/addGoodsAttribute'),       //注释掉    ---meng
    ...
    {
        if(IS_POST){
            $result = M('goods_integral')->where(['goods_id'=>$_SESSION['insertId']])->find();
            empty($_SESSION['insertId']) || $result ? $this->ajaxReturnData('数据错误', '请从头添加') : true;
            $data = [];
            ...
            $data['goods_id']    = $_SESSION['insertId'];
