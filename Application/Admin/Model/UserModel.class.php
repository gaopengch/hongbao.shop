...
class UserModel extends BaseModel implements IsExitsModel 
{
	...
	public static $accountBalance_d;	//余额
...
//        return $data;
//    }
    public function getUserByRecharge(array $data)
    {

      $data =  M('recharge')
                    ->alias('r')
                    ->field('r.*,u.user_name')
                    ->join('left join db_user AS u ON r.user_id = u.id')
                    ->select();
//        foreach ($data as $k => $v) {
//                $id = $v['user_id'];
//                $username = M('user')->where(['id'=>$id])->getField('user_name',true);
//                $data[$k]['user_name'] = $username['0'];
//        }
        return $data;
    }
    /**
