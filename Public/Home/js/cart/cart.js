...
function delete_many(obj) {
    ...
    $.get("/index.php/Home/Assets/collection_del",{'cart_id': id_str},function (ret) {
        if (ret.msg == 1) {
...
$('.ui-ceilinglamp .toolbar-wrap .toolbar-left input[type=checkbox]').on('click',function(){
    var list = $('#q_order_list input[type=checkbox]');
    var that = $(this);
    calculateTotal(that,list);
    // 添加样式
    //var dd = $('.cart .cart-main .cart-list dd');
    //dd.removeClass('active');
...
//全选计算总价
function calculateTotal(that,list){
    var total = 0;
    var goods_length = [];
    if(that.prop('checked') == true){
        list.each(function(){
            ...
            if(!$(this).prop("disabled")){
                ...
                goods_length.push($(this));
                $(this).parents('dd').removeClass('active').addClass('active');
                var price = $(this).parent().siblings('.p-price').find('b').text();
                var num = $(this).parent().siblings('.p-quantity').find('input').val();
                num = isNaN(parseInt(num))?0 :parseInt(num);
                var count = parseFloat(price.substring(1))*num;
                total += count;
            }
        })
        $('.ui-ceilinglamp .toolbar-right span').eq(0).find('em').text(goods_length.length);
        $('.ui-ceilinglamp .toolbar-right span').find('b').text(total.toFixed(2));
        $('.cart .cart-filter-bar .cart-store span b').text('￥'+total.toFixed(2));
    }else{
        list.each(function(){
        ...
        $('.ui-ceilinglamp .toolbar-right span').eq(0).find('em').text(0);
        $('.ui-ceilinglamp .toolbar-right span').find('b').text(total.toFixed(2));
        $('.cart .cart-filter-bar .cart-store span b').text('￥'+total.toFixed(2));
    }
    console.log(total)
