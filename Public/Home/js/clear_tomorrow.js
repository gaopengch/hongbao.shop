var tag = false;
$('.q_tomorrow_time').on('click','li',function(){
    if(tag == true ){return false;}
    tag = true;
    $(this).addClass('color').siblings().removeClass('color');
    var cid = $(this).find('.tomorrow_item_time').data('cid');
    var sid = $(this).find('.tomorrow_item_time').data('sid');
    var _this = $(this);
    $.get(GET_ClearGood,{cid: cid, sid: sid},function(res){
        ...
        if(res.data.goods.length == 0){
            ...
            for(var i=0; i < res.data.goods.length; i++){
                ...
                if(res.data.goods[i].is_customs == 0){
                    dom1 +=' </div><div class="q_join_cart q_btn" data-id='+goodsId+'>加入购物车</div>';
                }else{
                    dom1 +='</div><div class="q_to_bug q_btn">立即开抢</div>';
                }
